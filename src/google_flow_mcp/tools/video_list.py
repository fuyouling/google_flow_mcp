...
def register_video_list_tool(mcp: FastMCP) -> None:
    ...
    def video_list(
        project_id: Annotated[
            str,
            Field(description="Google Flow 项目的唯一 ID (UUID)。留空则自动从浏览器当前所在的项目页面中提取视频列表。")
        ] = ""
    ) -> str:
        """
        ...
        """
        logger.info(f"Executing video_list (project_id='{project_id}')")
        ...
        if is_busy:
            ...
            cached_videos = None
            if project_id:
                cached_videos = ProjectCache.get_project_videos(project_id)
            else:
        ...
        try:
            ...
            videos = page.list_videos(project_id=effective_project_id)

            # 3. 同步至本地缓存
            if effective_project_id:
                ProjectCache.update_project_videos(effective_project_id, videos)
