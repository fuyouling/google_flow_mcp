...
def register_image_list_tool(mcp: FastMCP) -> None:
    ...
    def image_list(
        project_name: Annotated[
            str,
            Field(description="Google Flow 项目的名称。留空则自动从浏览器当前所在的项目页面中提取图片列表。")
        ] = ""
    ) -> str:
        """
        查看并列出 Google Flow 项目中的所有图片（包括图片总数、图片名称与预览图）。
        
        特性：
        1. 若传入 project_name，则自动定位/跳转至该项目页面；若留空，则直接查看当前正在浏览器中打开的项目。
        2. 导航栏检查：判断若没有“图片”栏则直接返回空列表。
        3. 自动进行并发冲突保护：若当前后台有生成任务正在占用浏览器，工具会避免打断当前任务，并降级尝试返回此前本地缓存的数据。
        4. 每次成功抓取后，会自动同步更新本地缓存 projects_cache.json。
        """
        logger.info(f"Executing image_list (project_name='{project_name}')")
        ...
        try:
            ...
            images = page.list_images(project_url=project_url)

            # 3. 同步至本地缓存
            if effective_project_name:
                ProjectCache.update_project_images(effective_project_name, images)
