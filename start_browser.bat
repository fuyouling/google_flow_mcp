...
rem
rem 功能:
rem   复用项目现有配置 (.env 与 browser_config.yaml)，在配置指定的端口（默认 9222）常驻启动 Chromium 浏览器。
...
rem       start_browser.bat --status      - 查看当前配置端口浏览器及 CDP 运行状态
rem       start_browser.bat --force       - 强制重启（若端口冲突，先杀掉残留进程再启动）
rem       start_browser.bat --stop        - 关闭运行在配置端口的浏览器
...
title Google Flow MCP Browser Service
cd /d "%~dp0"
