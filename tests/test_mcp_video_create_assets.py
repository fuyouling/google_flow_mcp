r"""
通过 MCP Python 客户端测试 video_create 工具（素材参考模式 mode="asset"）
用途：验证视频生成流程中基于已有参考素材（如角色 Mary Lennox、Officer Barney）的创建流程及清晰度本地下载（270p/720p/1080p）保存流程
...
MODEL_NAME   = "Omni 1.1 Flash"  # 【重要限制】仅 Omni 1.1 Flash 模型支持引用素材生视频；Veo 系列在素材模式下不会引用参考素材
MODE         = "asset"           # 素材模式 (默认模式)
START_FRAME  = ""                # 素材模式下必须为空
END_FRAME    = ""                # 素材模式下必须为空
ASSETS       = "Mary Lennox,Officer Barney"  # 项目内已有角色素材名称
ASPECT_RATIO = "16:9"            # 可选: "16:9", "9:16"
RESOLUTION   = "360p"            # 仅 Omni 模型生效: "360p", "720p"
